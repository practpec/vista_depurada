...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (correo, contrasena) => {
    const response = await axios.post("http://34.205.236.98:4000/usuarios/login", {
      correo,
      contrasena,
    });
    ...
    const socket = io("https://ws-energywatch.onrender.com", {
      auth: { token },
    });
  ...
  const register = async (correo, nombre, contrasena) => {
    const response = await axios.post("http://34.205.236.98:4000/usuarios/one", {
      correo,
      nombre,
      contrasena,
    });
